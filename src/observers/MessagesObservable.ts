...
export class MessagesObservable {
    ...
    private async poll(): Promise<void> {
        try {
            if (this.subscriber) {
                const messages = (await this.messages.getMessages(this.idInbox)).filter(msg => new Date(msg.created_at) >= this.date);
    ...
    public subscribe(next?: (value: Message[]) => void, error?: (error: any) => void, complete?: () => void) {
