...
export class MessagesEndpoint implements IMessagesEndpoint {
    ...
    public async patchMessage(idInbox: number, idMessage: number, data: { message: Partial<Message> }): Promise<Message> {
    ...
    public async forwardMessage(idInbox: number, idMessage: number, data: { email: string }): Promise<MessageOnlyResult> {
        return await this.http.request('POST', `/inboxes/${idInbox}/messages/${idMessage}/forward`, data);
    }

    public async getMessageBody(idInbox: number, idMessage: number, type: "html" | "htmlsource" | "txt" | "raw" | "eml"): Promise<string> {
    ...
    public async getMessageSpamReport(idInbox: number, idMessage: number): Promise<MessageSpamReport> {
        return await this.http.request('GET', `/inboxes/${idInbox}/messages/${idMessage}/spam_report`);
    }

    public async getMessageAnalyze(idInbox: number, idMessage: number): Promise<MessageAnalyzeResult> {
        return await this.http.request('GET', `/inboxes/${idInbox}/messages/${idMessage}/analyze`);
    }

    public async getMessageAttachment(idInbox: number, idMessage: number, idAttachment: number): Promise<MessageAttachment> {
        return await this.http.request('GET', `/inboxes/${idInbox}/messages/${idMessage}/attachments/${idAttachment}`);
    }
